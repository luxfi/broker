...
// CreateJournal creates an inter-sub-account journal entry.
// In OmniSub, journals move cash (JNLC) or securities (JNLS) between
// sub-accounts or between the omnibus master and a sub-account.
...
// ListJournals lists journal entries, optionally filtered.
...
// GetJournal retrieves a single journal entry by ID.
...
// DeleteJournal cancels a pending journal entry.
...
// CreateBatchJournal creates up to 5000 journal entries in one call.
...
// ReverseBatchJournal reverses a batch of journal entries.
...
func parseJournal(data []byte) (*types.Journal, error) {
	var raw struct {
		ID          string `json:"id"`
